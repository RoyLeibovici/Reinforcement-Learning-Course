...
def value_iteration(mdp, gamma, nIt):
    ...
    for it in range(nIt):
        ...
        Vprevcopy = Vprev.copy() #Copy to avoid overriding
        V = np.zeros(mdp.nS)
        pi = np.zeros(mdp.nS)
        for state in range(mdp.nS):
            ...
            for action in range(mdp.nA):
              value_per_action = 0
              for option in mdp.P[state][action]: #Stochastic MDP has a probability of leading to different states per action
                  probability, next_state, reward = option
                  value_per_action += probability * (reward + gamma * Vprevcopy[next_state])

              possible_values.append(value_per_action) 

            max_value = max(possible_values) # Value calculated using the Bellman equations
            best_action = possible_values.index(max_value) #Policy is greedy w.r.t V(s)
            V[state] = max_value
...
#Plot the value for each state as a function of iteration
num_iterations = len(Vs_VI) - 1  
plt.figure(figsize=(12, 8))

for i in range(16):
    values = [Vs_VI[j][i] for j in range(num_iterations + 1)]
